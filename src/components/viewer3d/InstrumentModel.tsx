...
export const InstrumentModel: React.FC<InstrumentModelProps> = ({ instrument }) => {
  // If an authentic external photoreal GLB exists, use it
  if (instrument.modelCapability?.hasModel) {
    return (
  ...
  // Built-in interactive 3D anatomy models (immediate out-of-the-box 3D experience)
  const renderBuiltInModel = () => {
  ...
  return (
    <Suspense fallback={null}>
      <group>{renderBuiltInModel()}</group>
    </Suspense>
  );
};
